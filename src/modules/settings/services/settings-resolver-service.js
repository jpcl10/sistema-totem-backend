...
import { defaultGeneralSettings, defaultPrintingSettings, defaultPrintingSourceSettings, ensureStoreBelongsToOrganization, normalizeChannel, printingSources, sourceValue, toDateOnly } from './settings-shared.js';
...
export class SettingsResolverService {
    async execute({ organizationId, storeId, eventId, deviceId, channel, date }) {
        const normalizedChannel = normalizeChannel(channel);
        const [organization, settings, branding, printingSettings, store, event, device] = await Promise.all([
        ...
        const resolvedDate = toDateOnly(date ?? new Date());
        const [exception, weeklyHours, storeOperation] = await Promise.all([
        ...
        const businessHoursSource = exception
            ? 'EXCEPTION'
            : weeklyHours.length > 0
                ? store
                    ...
                    : 'ORGANIZATION'
                : 'DEFAULT';
        const legacyPrintingContext = event ?? device?.event ?? store ?? device?.store ?? null;
        const legacyPrintMode = legacyPrintingContext &&
        ...
        const legacyPrintingEnabled = legacyPrintingContext &&
        ...
        const legacyAutoPrintEnabled = legacyPrintingContext &&
        ...
        const legacyPaperSize = legacyPrintingContext &&
        ...
        const legacyPrintingFallback = {
            ...
            splitBySector: legacyPrintMode === 'BY_SECTOR' ||
