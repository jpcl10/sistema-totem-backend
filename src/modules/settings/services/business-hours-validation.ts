...
export function normalizeAndValidateBusinessHours(
  hours: BusinessHourInput[]
) {
  const normalized =
    hours.map(normalizePeriod)

  const grouped = new Map<number, BusinessHourInput[]>()

  for (const period of normalized) {
    const periods =
      grouped.get(period.dayOfWeek) ?? []

    if (
      periods.some(existing =>
        existing.periodIndex === period.periodIndex
      )
    ) {
  ...
  for (const periods of grouped.values()) {
    const fullDayPeriods =
      periods.filter(period =>
        period.isClosed || period.is24Hours
      )
    ...
    const regularPeriods =
      periods
        ...
        .sort((a, b) =>
          toMinutes(a.opensAt!) - toMinutes(b.opensAt!)
        )
...
export function normalizeAndValidateBusinessHourException(
  exception: BusinessHourExceptionInput
) {
