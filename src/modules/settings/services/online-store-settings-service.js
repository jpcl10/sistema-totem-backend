import { AuditAction, DeliveryFeeRuleType, OnlineOrderFulfillmentType, SettingsChannel, SettingsContextType } from '@prisma/client';
import { prisma } from '../../../lib/prisma.js';
import { CreateAuditLogService } from '../../audit-logs/services/create-audit-log-service.js';
import { defaultOnlineStoreSettings, ensureStoreBelongsToOrganization, normalizeChannel, normalizeNeighborhood } from './settings-shared.js';
...
function resolveAvailabilityAuditAction(previousMode, nextMode, previousUntil, nextUntil, previousReason, nextReason) {
    ...
    if (previousUntil?.toISOString() !== nextUntil?.toISOString() ||
...
function zonedLocalDateTimeToUtc(date, time, timezone) {
    const [hours, minutes] = time.split(':').map(Number);
    const target = Date.UTC(date.getUTCFullYear(), date.getUTCMonth(), date.getUTCDate(), hours, minutes);
...
function windowContainsNow(window, nowMinutes, fromPreviousDay = false) {
    if (window.isClosed) {
        return false;
    }
    if (window.is24Hours) {
...
function scheduleIsOpen(windows, dayOfWeek, previousDay, nowMinutes) {
...
function nextOpeningAt(weeklyWindows, localDate, dayOfWeek, nowMinutes, timezone, futureWindowsForDate) {
    for (let offset = 0; offset < 8; offset++) {
        const candidateDay = (dayOfWeek + offset) % 7;
        const candidateDate = addDays(localDate, offset);
        const candidateWindows = futureWindowsForDate?.(candidateDate, candidateDay);
        ...
        for (const window of dayWindows) {
            ...
            if (offset > 0 || opens > nowMinutes) {
                return zonedLocalDateTimeToUtc(candidateDate, window.is24Hours ? '00:00' : window.opensAt, timezone).toISOString();
            }
        }
    }
    return null;
}
function nextClosingAt(windows, localDate, dayOfWeek, previousDay, nowMinutes, timezone) {
    ...
    const closeDate = active.dayOfWeek === previousDay
        ? localDate
        : closes <= opens
            ? addDays(localDate, 1)
            : localDate;
...
function pickScopedException(exceptions, storeId, channel) {
    return exceptions.find(exception => exception.storeId === storeId &&
        exception.channel === channel) ??
        exceptions.find(exception => exception.storeId === storeId &&
            exception.channel === SettingsChannel.ALL) ??
        exceptions.find(exception => exception.storeId === null &&
            exception.channel === channel) ??
        exceptions.find(exception => exception.storeId === null &&
            exception.channel === SettingsChannel.ALL) ??
        null;
}
function exceptionToWindow(exception, dayOfWeek) {
...
export class OnlineStoreSettingsService {
    async getOrDefaults({ organizationId, storeId }) {
        const store = await ensureStoreBelongsToOrganization(organizationId, storeId);
        const settings = await prisma.onlineStoreSettings.findUnique({
            where: {
                storeId
            }
        });
    ...
    async updateOnlineOrders({ organizationId, userId, storeId, data }) {
        await ensureStoreBelongsToOrganization(organizationId, storeId);
        const settings = await prisma.onlineStoreSettings.upsert({
            where: {
                storeId
            },
            create: {
    ...
    async updateDelivery({ organizationId, userId, storeId, data }) {
        await ensureStoreBelongsToOrganization(organizationId, storeId);
        const settings = await prisma.onlineStoreSettings.upsert({
            where: {
                storeId
    ...
    async listDeliveryRules({ organizationId, storeId }) {
        await ensureStoreBelongsToOrganization(organizationId, storeId);
    ...
    async createDeliveryRule({ organizationId, userId, data }) {
        ...
        const rule = await prisma.deliveryFeeRule.create({
            data: {
                ...
                name: data.name,
                type: data.type,
                neighborhood: data.type === DeliveryFeeRuleType.NEIGHBORHOOD
    ...
    async updateDeliveryRule({ organizationId, userId, ruleId, data }) {
        const current = await prisma.deliveryFeeRule.findFirst({
            where: {
                id: ruleId,
                organizationId
            }
        ...
        const nextNeighborhood = data.neighborhood !== undefined
            ? data.neighborhood
            : current.neighborhood;
        if (nextType === DeliveryFeeRuleType.NEIGHBORHOOD &&
            !nextNeighborhood) {
        ...
        const rule = await prisma.deliveryFeeRule.update({
            ...
            data: {
                ...data,
                neighborhood: nextType === DeliveryFeeRuleType.NEIGHBORHOOD
                    ? nextNeighborhood
                    : null
            }
        });
    ...
    async deleteDeliveryRule({ organizationId, userId, ruleId }) {
        const rule = await prisma.deliveryFeeRule.findFirst({
            where: {
                id: ruleId,
                organizationId
            }
    ...
    async calculateDeliveryFee({ organizationId, storeId, subtotalInCents, neighborhood }) {
        const { effective } = await this.getOrDefaults({
            organizationId,
            storeId
        });
        const rules = await prisma.deliveryFeeRule.findMany({
        ...
        const neighborhoodRules = rules.filter(rule => rule.type === DeliveryFeeRuleType.NEIGHBORHOOD);
        const matchedNeighborhoodRule = normalizedNeighborhood
            ? neighborhoodRules.find(rule => normalizeNeighborhood(rule.neighborhood) === normalizedNeighborhood)
            : null;
        if (neighborhoodRules.length > 0 && !matchedNeighborhoodRule) {
            throw new Error('Delivery neighborhood is not served');
        }
        const flatRule = rules.find(rule => rule.type === DeliveryFeeRuleType.FLAT);
        const rule = matchedNeighborhoodRule ?? flatRule ?? null;
        const baseFee = rule?.feeInCents ?? effective.defaultDeliveryFeeInCents;
        const freeDeliveryAbove = rule?.freeDeliveryAboveInCents ??
            effective.freeDeliveryAboveInCents;
        const feeInCents = freeDeliveryAbove !== null &&
        ...
        return {
            ...
            minimumOrderInCents: rule?.minimumOrderInCents ??
                effective.minimumOrderInCents,
            freeDeliveryAboveInCents: freeDeliveryAbove
        };
    }
    async resolveOperation({ organizationId, storeId, channel, date, fulfillmentType, subtotalInCents, neighborhood }) {
        const store = await ensureStoreBelongsToOrganization(organizationId, storeId);
        if (!store) {
            throw new Error('Store not found');
        }
        const { settings, effective, source } = await this.getOrDefaults({
            organizationId,
            storeId
        });
        ...
        const timezone = organizationSettings?.timezone ?? DEFAULT_TIMEZONE;
        const localDate = zonedDateOnly(resolvedDate, timezone);
        const resolvedDay = zonedDayOfWeek(resolvedDate, timezone);
        const previousDay = (resolvedDay + 6) % 7;
        const nowLocal = zonedParts(resolvedDate, timezone);
        const nowMinutes = nowLocal.hour * 60 + nowLocal.minute;
        const futureLimitDate = addDays(localDate, 8);
        const [exceptions, storeWeeklyHours, organizationWeeklyHours] = await Promise.all([
        ...
        const todayException = pickScopedException(exceptions.filter(exception => localDateKey(exception.date) === localDateKey(localDate)), storeId, normalizedChannel);
        const exception = todayException;
        const weeklyHours = storeWeeklyHours.length > 0
            ? storeWeeklyHours
            : organizationWeeklyHours;
        const weeklyWindows = weeklyHours.map(hour => ({
        ...
        const scheduleSource = exception
            ...
            : storeWeeklyHours.length > 0
                ...
                : organizationWeeklyHours.length > 0
                    ? 'ORGANIZATION'
                    : 'DEFAULT';
        const rawOverrideMode = 'manualOverrideMode' in store
            ? String(store.manualOverrideMode)
            : 'AUTO';
        const overrideUntil = 'manualOverrideUntil' in store
            ? store.manualOverrideUntil ?? null
            : null;
        const overrideExpired = overrideUntil !== null &&
            overrideUntil <= resolvedDate;
        const manualOverride = overrideExpired
            ? 'AUTO'
            : rawOverrideMode;
        ...
        const legacyManualClosed = false;
        const openNow = store.active &&
            (manualOverride === 'FORCE_OPEN' ||
                (manualOverride === 'AUTO' &&
                    scheduleOpen));
        const windowsForDate = (date, candidateDay) => {
            const futureException = pickScopedException(exceptions.filter(candidate => localDateKey(candidate.date) === localDateKey(date)), storeId, normalizedChannel);
            return futureException
                ? exceptionToWindow(futureException, candidateDay)
                : null;
        };
        const nextOpening = openNow
            ? null
            : nextOpeningAt(weeklyWindows, localDate, resolvedDay, nowMinutes, timezone, windowsForDate);
        const nextClosing = scheduleOpen
        ...
        const requestedFulfillment = fulfillmentType ?? OnlineOrderFulfillmentType.DELIVERY;
        ...
        if (requestedFulfillment === OnlineOrderFulfillmentType.DELIVERY &&
        ...
        const acceptingOrders = unavailableReason === null &&
        ...
        return {
            ...
            delivery: {
                ...
                estimatedMinutes: requestedFulfillment === OnlineOrderFulfillmentType.DELIVERY
    ...
    async updateAvailability({ organizationId, userId, storeId, mode, until, reason }) {
        await ensureStoreBelongsToOrganization(organizationId, storeId);
        const currentStore = await prisma.onlineStore.findFirst({
            where: {
                id: storeId,
                organizationId
            },
        ...
        const [organizationSettings, configuredHours] = await Promise.all([
        ...
        const previousMode = String(currentStore.manualOverrideMode);
        ...
        await new CreateAuditLogService().execute({
            ...
            entity: 'OnlineStore',
            entityId: storeId,
            action: resolveAvailabilityAuditAction(previousMode, mode, currentStore.manualOverrideUntil, nextUntil, currentStore.manualOverrideReason, nextReason),
