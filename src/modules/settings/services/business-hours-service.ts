...
import {
  normalizeAndValidateBusinessHourException,
  normalizeAndValidateBusinessHours
} from './business-hours-validation.js'
import {
...
type UpsertBusinessHoursRequest = {
...
type CreateBusinessHourExceptionRequest = {
  organizationId: string
  userId: string
  storeId?: string | null
  channel?: SettingsChannel
...
type UpdateBusinessHourExceptionRequest = {
  organizationId: string
  userId: string
  exceptionId: string
  storeId?: string | null
...
type DeleteBusinessHourExceptionRequest = {
  organizationId: string
  userId: string
  exceptionId: string
}
...
export class BusinessHoursService {
  ...
  async upsert({
    ...
    const normalizedHours =
      normalizeAndValidateBusinessHours(hours)
  ...
  async createException({
    ...
    const exception =
      await prisma.businessHourException.create({
        data: {
          organizationId,
  ...
  async updateException(request: UpdateBusinessHourExceptionRequest) {
    const current =
      await prisma.businessHourException.findFirst({
        where: {
    ...
    const isClosed =
      request.isClosed ?? current.isClosed

    const is24Hours =
      request.is24Hours ?? current.is24Hours

    const opensAt =
      request.opensAt !== undefined
        ? request.opensAt
        : current.opensAt

    const closesAt =
      request.closesAt !== undefined
        ? request.closesAt
        : current.closesAt
    ...
    const exception =
      await prisma.businessHourException.update({
        where: {
          id: request.exceptionId
        },
        data: {
  ...
  async deleteException({
    ...
    userId,
    exceptionId
  }: DeleteBusinessHourExceptionRequest) {
    const exception =
      await prisma.businessHourException.findFirst({
        where: {
          id: exceptionId,
