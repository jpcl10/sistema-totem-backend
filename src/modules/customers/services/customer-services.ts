...
import {
  ...
  normalizePhone
} from '../utils/customer-normalization.js'
...
async function assertNoDuplicateCustomerIdentifiers({
  organizationId,
  normalizedPhone,
  normalizedEmail,
  normalizedDocument,
...
export class CreateCustomerService {
  async execute({ organizationId, userId, data }: Actor & { data: CustomerInput }) {
...
export class GetCustomerService {
  async execute({ organizationId, customerId }: { organizationId: string; customerId: string }) {
...
export class UpdateCustomerService {
  async execute({
    ...
    customerId,
    data
  }: Actor & { customerId: string; data: CustomerInput }) {
    ...
    const normalizedPhone =
      data.phone !== undefined ? normalizePhone(data.phone) : current.normalizedPhone
    const normalizedEmail =
      data.email !== undefined ? normalizeEmail(data.email) : current.normalizedEmail
    const normalizedDocument =
      data.document !== undefined ? normalizeDocument(data.document) : current.normalizedDocument
...
export class UpdateCustomerStatusService {
  async execute({
    ...
    customerId,
    active
  }: Actor & { customerId: string; active: boolean }) {
...
export class CreateCustomerAddressService {
  async execute({
    ...
    customerId,
    data
  }: Actor & { customerId: string; data: AddressInput }) {
    const customer = await prisma.customer.findFirst({
      where: { id: customerId, organizationId },
      select: { id: true }
    })
...
export class UpdateCustomerAddressService {
  async execute({
    ...
    addressId,
    data
  }: Actor & { customerId: string; addressId: string; data: AddressInput }) {
