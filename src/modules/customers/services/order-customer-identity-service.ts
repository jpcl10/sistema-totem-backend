...
function buildAddressLabel(label?: string | null) {
...
export class ResolveOrderCustomerIdentityService {
  ...
  }: ResolveOrderCustomerIdentityRequest) {
    const requestedName =
      customer?.name?.trim() || fallbackCustomerName?.trim() || null
