...
import { getPeriodDateFilter } from '../../../shared/utils/get-period-date-filter.js';
export class GetEventMetricsService {
    async execute({ organizationId, eventId, period = 'EVENT', startDate, endDate }) {
        ...
        const totalRevenueInCents = orders.reduce((sum, order) => {
            return sum + order.totalInCents;
        }, 0);
        const averageTicketInCents = totalOrders > 0
        ...
        for (const order of orders) {
            for (const item of order.items) {
                const sector = item.catalogProduct
                    ?.catalogCategory
                    ?.sector;
                if (sector === 'BAR' ||
                    sector === 'KITCHEN') {
                ...
                const key = item.catalogProductId ||
                    item.productId ||
                    item.productName;
        ...
        const topProducts = Array
            ...
            return (secondProduct.quantity -
                firstProduct.quantity);
        })
            .slice(0, 10);
        const totalProductsSold = orders.reduce((ordersTotal, order) => {
        ...
        return {
            metrics: {
                ...
                dateRange: createdAtFilter
                    ? {
                        startDate: createdAtFilter.gte.toISOString(),
                        endDate: createdAtFilter.lte.toISOString()
                    }
                    : null,
                totalOrders,
                totalProductsSold,
