...
import {
  MarkUnifiedOrderPaymentService,
  UnifiedPaymentOrderType
} from '../services/mark-unified-order-payment-service.js'
...
export async function markUnifiedOrderPaymentController(
  request: FastifyRequest,
  reply: FastifyReply
) {
  try {
    const { orderType, orderId } =
      markUnifiedOrderPaymentParamsSchema.parse(request.params)
    ...
    if (error instanceof Error) {
      ...
      if (
        error.message === 'Amount received cannot be negative' ||
