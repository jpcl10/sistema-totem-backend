import { DeviceType, OnlineOrderPaymentMethod, OnlineOrderStatus, OrderSource, OrderStatus, PaymentMethod, PaymentStatus, Prisma } from '@prisma/client';
import { prisma } from '../../../lib/prisma.js';
const fallbackTimezone = 'America/Sao_Paulo';
function getTimeZoneOffsetInMs(date, timeZone) {
    ...
    const localTimeAsUtc = Date.UTC(values.year, values.month - 1, values.day, values.hour, values.minute, values.second);
    const dateWithoutMilliseconds = Math.floor(date.getTime() / 1000) * 1000;
    return localTimeAsUtc - dateWithoutMilliseconds;
}
function zonedTimeToUtc(timeZone, year, month, day, hour, minute, second, millisecond) {
...
function getFinancialDateRange({ period, startDate, endDate, timeZone, now = new Date() }) {
    if (period === 'EVENT') {
        return undefined;
    }
    if (period === 'YESTERDAY') {
        const current = getZonedDateParts(now, timeZone);
        const todayStart = zonedTimeToUtc(timeZone, current.year, current.month, current.day, 0, 0, 0, 0);
        const yesterday = new Date(todayStart.getTime() - 24 * 60 * 60 * 1000);
        const yesterdayParts = getZonedDateParts(yesterday, timeZone);
        return {
            gte: zonedTimeToUtc(timeZone, yesterdayParts.year, yesterdayParts.month, yesterdayParts.day, 0, 0, 0, 0),
            lte: zonedTimeToUtc(timeZone, yesterdayParts.year, yesterdayParts.month, yesterdayParts.day, 23, 59, 59, 999)
        };
    }
    if (period === '24H') {
        return {
    ...
    if (period === 'CUSTOM') {
        ...
        return {
            gte: zonedTimeToUtc(timeZone, start.year, start.month, start.day, start.hour, start.minute, start.second, start.millisecond),
            lte: zonedTimeToUtc(timeZone, end.year, end.month, end.day, end.hour, end.minute, end.second, end.millisecond)
        };
    }
    const current = getZonedDateParts(now, timeZone);
    return {
        gte: zonedTimeToUtc(timeZone, current.year, current.month, current.day, 0, 0, 0, 0),
        lte: zonedTimeToUtc(timeZone, current.year, current.month, current.day, 23, 59, 59, 999)
    };
}
function normalizeEventPaymentMethod(method) {
    switch (method) {
...
function normalizeOnlinePaymentMethod(method) {
    switch (method) {
...
function addAmount(map, key, value) {
    map[key] = (map[key] ?? 0) + value;
}
function addPaymentBucket(byPaymentMethod, normalizedMethod, originalMethod, amountInCents, ordersCount) {
...
function shouldIncludeEventOrders(request) {
...
function shouldIncludeOnlineOrders(request) {
...
function buildEventWhere({ organizationId, eventId, source, paymentMethod, paymentStatus, dateField, dateRange }) {
...
function buildOnlineWhere({ organizationId, storeId, source, paymentMethod, paymentStatus, fulfillmentType, dateField, dateRange }) {
    const normalizedPaymentMethod = paymentMethodFilter(paymentMethod);
    const sourceFilter = source === 'MANUAL_STORE'
        ? OrderSource.ADMIN
...
function getTimeseriesGranularity({ period, dateRange }) {
    ...
    if (period === 'CUSTOM' && dateRange) {
        const days = (dateRange.lte.getTime() - dateRange.gte.getTime()) /
            (24 * 60 * 60 * 1000);
        if (days <= 2) {
            return 'HOUR';
        }
...
function getTimeseriesStep(granularity) {
    switch (granularity) {
        case 'HOUR':
            return '1 hour';
        case 'MONTH':
...
function getDateTruncGranularity(granularity) {
    switch (granularity) {
        case 'HOUR':
            return 'hour';
        case 'MONTH':
...
function formatTimeseriesLabel(date, timezone, granularity) {
    if (granularity === 'HOUR') {
...
function buildEventTimeseriesConditions(request) {
    ...
    if (normalizedPaymentMethod &&
        normalizedPaymentMethod in PaymentMethod) {
        conditions.push(Prisma.sql `o."paymentMethod" = ${normalizedPaymentMethod}::"PaymentMethod"`);
    }
...
function buildOnlineTimeseriesConditions(request) {
    const normalizedPaymentMethod = paymentMethodFilter(request.paymentMethod);
    const conditions = [
    ...
    const sourceFilter = request.source === 'MANUAL_STORE'
        ? OrderSource.ADMIN
    ...
    if (request.fulfillmentType) {
        conditions.push(Prisma.sql `oo."fulfillmentType" = ${request.fulfillmentType}::"OnlineOrderFulfillmentType"`);
    }
    if (sourceFilter) {
    ...
    if (normalizedPaymentMethod &&
        normalizedPaymentMethod in OnlineOrderPaymentMethod) {
        conditions.push(Prisma.sql `oo."paymentMethod" = ${normalizedPaymentMethod}::"OnlineOrderPaymentMethod"`);
    }
    else if (normalizedPaymentMethod) {
        conditions.push(Prisma.sql `false`);
    }
...
async function buildTimeseries({ request, dateRange, timezone, period, includeEventOrders, includeOnlineOrders }) {
    ...
    return {
        ...
        points: rows.map(row => {
            ...
            return {
                ...
                grossRevenueInCents,
                paidOrdersCount,
                averageTicketInCents: paidOrdersCount > 0
...
export class FinancialAggregationService {
    async execute(request) {
        ...
        const includePaidRevenue = !request.paymentStatus ||
            request.paymentStatus === PaymentStatus.PAID;
        const includePendingCount = !request.paymentStatus ||
            request.paymentStatus === PaymentStatus.PENDING;
        const includeCancelledCount = !request.paymentStatus ||
            request.paymentStatus === PaymentStatus.CANCELLED;
        const includeRefundedCount = !request.paymentStatus ||
        ...
        const refundedOnlineWhere = includeOnlineOrders && includeRefundedCount
            ...
            : null;
        const [paidEventAggregate, paidOnlineAggregate, deliveryFeeAggregate, refundedEventAggregate, refundedOnlineAggregate, paidEventCount, paidOnlineCount, pendingEventCount, pendingOnlineCount, cancelledEventCount, cancelledOnlineCount, refundedEventCount, refundedOnlineCount, eventPaymentGroups, onlinePaymentGroups, onlineSourceGroups, onlineFulfillmentGroups, eventManualCount, eventTotemCount, eventDefaultCount] = await Promise.all([
            paidEventWhere
        ...
        const grossRevenueInCents = (paidEventAggregate._sum.totalInCents ?? 0) +
            (paidOnlineAggregate._sum.totalInCents ?? 0);
        const refundsInCents = (refundedEventAggregate._sum.totalInCents ?? 0) +
        ...
        for (const group of eventPaymentGroups) {
            ...
            addPaymentBucket(byPaymentMethod, normalizeEventPaymentMethod(group.paymentMethod), originalMethod, group._sum.totalInCents ?? 0, group._count._all);
        }
        for (const group of onlinePaymentGroups) {
            const originalMethod = group.paymentMethod ?? 'OTHER';
            addPaymentBucket(byPaymentMethod, normalizeOnlinePaymentMethod(group.paymentMethod), originalMethod, group._sum.totalInCents ?? 0, group._count._all);
        }
        ...
        for (const group of onlineSourceGroups) {
            const source = group.source === OrderSource.ADMIN
                ? 'MANUAL_STORE'
        ...
        for (const group of onlineFulfillmentGroups) {
            addAmount(byFulfillmentType, group.fulfillmentType, group._sum.totalInCents ?? 0);
        }
        ...
        return {
            summary: {
                ...
                deliveryFeesInCents: deliveryFeeAggregate._sum.deliveryFeeInCents ?? 0,
                discountsInCents: 0,
                refundsInCents,
                paidOrdersCount,
