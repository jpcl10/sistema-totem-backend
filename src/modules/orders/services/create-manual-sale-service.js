import { AuditAction, CustomerSource, PaymentStatus, OrderStatus } from '@prisma/client';
...
export class CreateManualSaleService {
    async execute({ organizationId, userId, eventId, customerName, customerId, paymentMethod, paymentStatus, items }) {
        ...
        const order = await prisma.$transaction(async (tx) => {
            ...
            for (const item of items) {
                ...
                if (eventProduct.trackStock &&
                    ...
                    throw new Error(`Insufficient stock for ${eventProduct.catalogProduct.name}`);
                }
            }
            const eventProductById = new Map(eventProducts.map(ep => [ep.id, ep]));
            const { orderItemsData, subtotalInCents: totalInCents } = await buildConfigurableCatalogOrderItems({
                tx,
                organizationId,
                items: items.map(item => {
                    ...
                    return {
                        ...
                        basePriceInCents: eventProduct.priceInCents ??
