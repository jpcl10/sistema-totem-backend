...
type PublicCallScreenStatus = 'PREPARING' | 'READY'
type PublicCallScreenFulfillmentType = 'PICKUP' | 'COUNTER' | 'ON_SITE' | 'EVENT'

type PublicCallScreenOrderDTO = {
  id: string
  publicCode: string
  orderNumber: number | null
  status: PublicCallScreenStatus
  statusLabel: string
  fulfillmentType: PublicCallScreenFulfillmentType
  displayName: string | null
  updatedAt: string
  readyAt: string | null
}

type PublicCallScreenOrders = {
  preparing: PublicCallScreenOrderDTO[]
  ready: PublicCallScreenOrderDTO[]
}
...
function publicCode(orderNumber: number | null | undefined) {
  return orderNumber ? `#${orderNumber}` : '#--'
}

function statusLabel(status: PublicCallScreenStatus) {
  return status === 'READY' ? 'Pronto' : 'Em preparo'
}

function mapEventStatus(status: OrderStatus): PublicCallScreenStatus | null {
...
function mapOnlineStatus(status: OnlineOrderStatus): PublicCallScreenStatus | null {
...
function mapOnlineFulfillmentType(
  fulfillmentType: OnlineOrderFulfillmentType
): PublicCallScreenFulfillmentType | null {
...
function splitOrders(orders: PublicCallScreenOrderDTO[]): PublicCallScreenOrders {
  return {
    preparing: orders
...
export class PublicCallScreenService {
  async getBootstrap(request: PublicCallScreenServiceRequest) {
  ...
  async getOrders(request: PublicCallScreenServiceRequest) {
    const context = await this.resolveContext(request)
  ...
  private async resolveBranding(
    context: Awaited<ReturnType<PublicCallScreenService['resolveContext']>>
  ) {
    ...
    return {
      ...
      primaryColor:
        effective.branding.primaryColor.value ??
        ('primaryColor' in context ? context.primaryColor : null) ??
        null,
      secondaryColor:
        effective.branding.secondaryColor.value ??
        ('secondaryColor' in context ? context.secondaryColor : null) ??
        null,
      backgroundColor:
        effective.branding.backgroundColor.value ??
  ...
  private async listOrdersForContext(
    context: Awaited<ReturnType<PublicCallScreenService['resolveContext']>>
  ) {
