import { PaymentMethod, PaymentProvider, PaymentStatus, PaymentTransactionStatus, AuditAction } from '@prisma/client';
...
export class MarkOrderPaymentService {
    async execute({ organizationId, userId, orderId, paymentStatus, paymentMethod, amountPaidInCents, changeForInCents, paymentNotes }) {
        ...
        const shouldMarkAsPaid = paymentStatus === PaymentStatus.PAID;
        if (shouldMarkAsPaid &&
        ...
        const paidAmount = shouldMarkAsPaid
                ...
                if (existingApprovedTransaction === 0) {
                    await tx.paymentTransaction.create({
                        data: {
                            ...
                            gatewayMessage: paymentNotes ?? 'Pagamento manual aprovado',
                            approvedAt: now,
                            metadata: {
