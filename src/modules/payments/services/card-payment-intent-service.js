import { AuditAction, PaymentContextType, PaymentStatus, PaymentTransactionStatus } from '@prisma/client';
...
export class CardPaymentIntentService {
    async create({ organizationId, orderId, onlineOrderId, terminalId, method, provider, amountInCents, installments, idempotencyKey }) {
    ...
    async confirm({ organizationId, deviceId, paymentTransactionId, result, amountInCents, providerTransactionId, authorizationCode, nsu, brand, installments, gatewayMessage }) {
        ...
        const paymentTransaction = await prisma.$transaction(async (tx) => {
            const updated = await tx.paymentTransaction.update({
                ...
                data: {
                    ...
                    approvedAt: nextStatus === PaymentTransactionStatus.APPROVED
                        ? now
                        : transaction.approvedAt,
                    rejectedAt: nextStatus === PaymentTransactionStatus.REJECTED
                        ? now
                        : transaction.rejectedAt,
                    cancelledAt: nextStatus === PaymentTransactionStatus.CANCELLED
                        ? now
                        : transaction.cancelledAt,
                    errorMessage: nextStatus === PaymentTransactionStatus.ERROR
            ...
            if (transaction.orderId) {
                await tx.order.update({
                    ...
                    data: {
                        paymentStatus: nextStatus === PaymentTransactionStatus.APPROVED
                            ? PaymentStatus.PAID
                            : nextStatus === PaymentTransactionStatus.WAITING_PAYMENT
                                ? PaymentStatus.PENDING
                                : PaymentStatus.FAILED,
                        paymentMethod: transaction.method,
                        amountPaidInCents: nextStatus === PaymentTransactionStatus.APPROVED
                            ? transaction.amountInCents
                            : null,
                        paidAt: nextStatus === PaymentTransactionStatus.APPROVED
                            ? now
                            : null,
        ...
        await new CreateAuditLogService().execute({
            ...
            action: nextStatus === PaymentTransactionStatus.APPROVED
