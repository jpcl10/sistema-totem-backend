...
export class UpdatePaymentTransactionStatusService {
    async execute({ organizationId, paymentTransactionId, status, gatewayStatus, gatewayMessage, errorMessage, metadata }) {
            ...
            if (status === PaymentTransactionStatus.APPROVED) {
                await tx.order.update({
                    ...
                    data: {
                        ...
                        paymentMethod: transaction.method ?? PaymentMethod.OTHER,
                        amountPaidInCents: transaction.amountInCents,
                        paidAt: now,
                        paymentNotes: gatewayMessage ??
                            transaction.gatewayMessage ??
                            'Pagamento aprovado'
                    }
                });
            }
            if (status === PaymentTransactionStatus.REJECTED ||
