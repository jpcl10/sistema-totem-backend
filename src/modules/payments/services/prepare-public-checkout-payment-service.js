import { OrderStatus, PaymentMethod, PaymentProvider, PaymentStatus, PaymentTransactionStatus } from '@prisma/client';
...
export class PreparePublicCheckoutPaymentService {
    async execute({ orderId }) {
        ...
        const isPaymentConfirmed = order.paymentStatus === PaymentStatus.PAID ||
        ...
        const pixAutomaticAvailable = Boolean(mercadoPagoSettings?.enabled &&
        ...
        const existingWaitingTransaction = await prisma.paymentTransaction.findFirst({
            where: {
                orderId: order.id,
                provider: PaymentProvider.MERCADO_PAGO,
        ...
        if (paymentTransaction.status === PaymentTransactionStatus.WAITING_PAYMENT &&
