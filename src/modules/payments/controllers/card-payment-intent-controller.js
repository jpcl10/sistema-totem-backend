...
import { z } from 'zod';
import { CardPaymentIntentService } from '../services/card-payment-intent-service.js';
...
const confirmCardPaymentIntentBodySchema = forbiddenCardDataSchema.and(z.object({
    result: z.enum([
        'APPROVED',
        'DECLINED',
        'CANCELLED',
...
function mapControllerError(reply, error) {
...
export async function createCardPaymentIntentController(request, reply) {
    try {
...
export async function confirmCardPaymentIntentController(request, reply) {
    try {
        const { paymentTransactionId } = confirmCardPaymentIntentParamsSchema.parse(request.params);
