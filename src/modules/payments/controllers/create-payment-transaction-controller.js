...
export async function createPaymentTransactionController(request, reply) {
    const { orderId } = createPaymentTransactionParamsSchema.parse(request.params);
    const { provider, method, amountInCents, externalReference, gatewayStatus, gatewayMessage, metadata } = createPaymentTransactionBodySchema.parse(request.body);
