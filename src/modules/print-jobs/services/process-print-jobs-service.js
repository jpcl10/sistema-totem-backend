...
function buildPrintContent(payload) {
    let content = '';
    content += '==============================\n';
...
export class ProcessPrintJobsService {
    ...
    async processOne({ printJobId, workerId, attemptNumber, maxAttempts = 5 }) {
        const staleLockThreshold = new Date(Date.now() - printProcessingConfig.staleLockMs);
        ...
        if (existing.status === PrintJobStatus.COMPLETED ||
        ...
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : 'Print error';
    ...
    async failClaimedJob({ printJobId, workerId, errorMessage, finalFailure, organizationId, eventId }) {
