...
export type UploadAssetType =
  typeof uploadAssetTypes[number]

export type UploadOutputFormat =
  'webp' | 'png'

export type UploadAssetProfile = {
  label: string
...
function profile(
  data: Omit<
    UploadAssetProfile,
    'allowedMimeTypes' | 'maxFileSizeInBytes'
  > & {
    maxFileSizeInMB: number
  }
): UploadAssetProfile {
...
export const uploadAssetProfiles: Record<
  UploadAssetType,
  UploadAssetProfile
> = {
  generic: profile({
...
const uploadAssetAliases: Record<string, UploadAssetType> = {
...
export const uploadMaxFileSizeInBytes =
  Math.max(
    ...Object.values(uploadAssetProfiles)
      .map(profile => profile.maxFileSizeInBytes)
  )

export const uploadMaxFileSizeInMB =
  uploadMaxFileSizeInBytes / bytesPerMB

export const uploadMultipartLimits = {
  fileSize: uploadMaxFileSizeInBytes
}

export const sharpLimitInputPixels =
  48_000_000

export function formatUploadLimitMessage(
  profileOrMaxMB: UploadAssetProfile | number = uploadMaxFileSizeInMB
) {
...
export function normalizeUploadAssetType(
  value: unknown
): UploadAssetType {
  if (
    typeof value !== 'string' ||
  ...
  if (
    uploadAssetTypes.includes(
      normalized as UploadAssetType
    )
  ) {
...
export function getUploadOutputContentType(
  outputFormat: UploadOutputFormat
) {
  if (outputFormat === 'png') {
    return 'image/png'
...
export function getUploadOutputExtension(
  outputFormat: UploadOutputFormat
) {
  if (outputFormat === 'png') {
    return 'png'
