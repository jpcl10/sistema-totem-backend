...
interface RefundNfcCardServiceRequest {
...
export class RefundNfcCardService {
  async execute({
    organizationId,
    userId,
    eventId,
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const transaction = await tx.nfcCardTransaction.create({
        data: {
          ...
          type: NfcCardTransactionType.REFUND,
          amountInCents,
    ...
    await createAuditLogService.execute({
      ...
      action: AuditAction.NFC_BALANCE_REFUND,
      description: description || 'Estorno de saldo',
      metadata: {
