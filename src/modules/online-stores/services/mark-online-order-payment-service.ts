...
interface MarkOnlineOrderPaymentServiceRequest {
  organizationId: string
  userRole: UserRole
  userId: string
  orderId: string
  paymentStatus: PaymentStatus
  paymentMethod?: OnlineOrderPaymentMethod | null
...
export class MarkOnlineOrderPaymentService {
  async execute({
    ...
    changeInCents
  }: MarkOnlineOrderPaymentServiceRequest) {
    ...
    if (
      effectivePaymentMethod === OnlineOrderPaymentMethod.CASH &&
      amountReceivedInCents !== undefined &&
      amountReceivedInCents !== null
    ) {
