...
export class GetPublicStoreService {
    async execute({ slug }) {
        ...
        for (const product of products) {
            ...
            if (categoriesMap.has(product.catalogCategoryId)) {
                categoriesMap.get(product.catalogCategoryId)?.products.push({
                    ...
                    halfAndHalfFlavors: product.supportsHalfAndHalf
                        ? products
        ...
        const logoUrl = effective.branding.logoUrl.value ?? store.logoUrl;
        const bannerUrl = effective.branding.bannerUrl.value ?? store.bannerUrl;
        return {
            store: {
                id: store.id,
                name: store.name,
            ...
            operation: {
                ...
                acceptingOrders: operation.availability.acceptingOrders,
                statusMessage: operation.availability.acceptingOrders
                    ? 'Aberto agora'
                ...
                nextOpeningAt: operation.availability.nextOpeningAt,
                nextClosingAt: operation.availability.nextClosingAt,
                estimatedPreparationMinutes: operation.onlineOrders.estimatedPreparationMinutes,
                estimatedDeliveryMinutes: operation.delivery.estimatedDeliveryMinutes
            },
            fulfillment: {
            ...
            orderRules: {
                ...
                allowOrdersOutsideHours: operation.onlineOrders.allowOrdersOutsideHours,
