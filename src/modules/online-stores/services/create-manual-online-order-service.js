import { CustomerSource, OnlineOrderFulfillmentType, OnlineOrderPaymentMethod, OnlineOrderStatus, OrderSource, PaymentStatus, SettingsChannel } from '@prisma/client';
...
export class CreateManualOnlineOrderService {
    async execute(request) {
        ...
        const order = await prisma.$transaction(async (tx) => {
            ...
            const deliverySnapshot = request.fulfillment === 'DELIVERY' && request.delivery
                ? {
            ...
            const finalDeliverySnapshot = request.fulfillment === OnlineOrderFulfillmentType.DELIVERY
                ? {
            ...
            const operation = await new OnlineStoreSettingsService().resolveOperation({
                organizationId: request.organizationId,
                storeId: store.id,
                channel: request.fulfillment === OnlineOrderFulfillmentType.DELIVERY
            ...
            const createdOrder = await tx.onlineOrder.create({
                data: {
                    ...
                    paidAt: request.paymentStatus === PaymentStatus.PAID
                        ? new Date()
                        : null,
