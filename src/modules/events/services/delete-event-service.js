...
export class DeleteEventService {
    async execute({ eventId, organizationId, userRole }) {
        ...
        const paymentTransactionsCount = await prisma.paymentTransaction.count({
            where: {
                order: {
                    eventId: event.id
                }
        ...
        const hasOperationalData = event._count.orders > 0 ||
            event._count.printJobs > 0 ||
            paymentTransactionsCount > 0 ||
            Boolean(event.closing);
        if (hasOperationalData) {
            throw new Error('Event cannot be deleted because it has operational data. Archive it instead.');
        }
