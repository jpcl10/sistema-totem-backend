...
export class GetPublicEventCatalogMenuService {
    async execute({ organizationSlug, eventSlug }) {
        ...
        const logoUrl = effective.branding.logoUrl.value ?? event.logoUrl;
        const bannerUrl = effective.branding.bannerUrl.value ?? event.bannerUrl;
        return {
            event: {
                id: event.id,
                name: event.name,
