import { AuditAction, PaymentContextType, PaymentEnvironment, PaymentProvider, Prisma, UserRole } from '@prisma/client';
import { z } from 'zod';
import { prisma } from '../../lib/prisma.js';
import { verifyJWT } from '../auth/middlewares/verify-jwt.js';
import { getTenantOrganizationId, requireTenantContext } from '../auth/middlewares/request-context.js';
...
function assertCanEdit(request) {
...
async function mapError(reply, error) {
...
async function getOrganizationPaymentSettings(request, reply) {
    try {
...
async function patchOrganizationPaymentSettings(request, reply) {
    try {
...
async function patchProviderCredentials(request, reply) {
    try {
        assertCanEdit(request);
        const organizationId = getTenantOrganizationId(request);
        ...
        const credential = await prisma.paymentProviderCredential.upsert({
            ...
            create: {
                ...
                publicMetadata: body.publicMetadata === null
                    ? Prisma.JsonNull
                    : body.publicMetadata,
...
async function getEffectivePaymentSettings(request, reply) {
    try {
        const organizationId = getTenantOrganizationId(request);
...
async function upsertEventContextSettings(request, reply) {
    try {
        assertCanEdit(request);
        const organizationId = getTenantOrganizationId(request);
...
async function upsertOnlineStoreContextSettings(request, reply) {
    try {
        assertCanEdit(request);
        const organizationId = getTenantOrganizationId(request);
...
async function upsertPaymentTerminal(request, reply) {
    try {
        assertCanEdit(request);
        const organizationId = getTenantOrganizationId(request);
...
async function listPaymentTerminals(request, reply) {
    const organizationId = getTenantOrganizationId(request);
...
export async function paymentSettingsRoutes(app) {
