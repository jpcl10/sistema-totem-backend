...
export interface PaymentSettingsContextInput {
...
export interface SafeEffectivePaymentSettings {
  organizationId: string
...
function clampPixExpiration(value: number) {
  return Math.min(Math.max(value, 2), 60)
}

function clampInstallments(value: number) {
...
export class PaymentSettingsResolver {
  async resolve({
    ...
    eventId,
    onlineStoreId
  }: PaymentSettingsContextInput): Promise<SafeEffectivePaymentSettings> {
    ...
    const legacyProviderSettings =
      await prisma.paymentProviderSettings.findMany({
        where: {
          organizationId
        },
        select: {
    ...
    const legacyMercadoPago =
      legacyProviderSettings.find(
        setting => setting.provider === PaymentProvider.MERCADO_PAGO
      )

    const base = organizationSettings ?? {
      organizationId,
      pixEnabled:
        Boolean(legacyMercadoPago?.enabled && legacyMercadoPago.pixEnabled),
      creditEnabled:
        Boolean(legacyMercadoPago?.enabled && legacyMercadoPago.cardEnabled),
      debitEnabled:
        Boolean(legacyMercadoPago?.enabled && legacyMercadoPago.cardEnabled),
      cashEnabled: true,
      nfcBalanceEnabled: false,
      defaultProvider:
        legacyMercadoPago?.enabled
    ...
    const providerCredentials =
      await prisma.paymentProviderCredential.findMany({
        where: {
          organizationId,
    ...
    const providers = new Map<PaymentProvider, {
      provider: PaymentProvider
      active: boolean
      configured: boolean
      environment: PaymentEnvironment
    ...
    return {
      ...
      methods: {
        pix: applyOverride(
          base.pixEnabled,
          contextSettings?.pixEnabledOverride
        ),
        credit: applyOverride(
          base.creditEnabled,
          contextSettings?.creditEnabledOverride
        ),
        debit: applyOverride(
          base.debitEnabled,
          contextSettings?.debitEnabledOverride
        ),
        cash: applyOverride(
          base.cashEnabled,
          contextSettings?.cashEnabledOverride
        ),
        nfcBalance: applyOverride(
          base.nfcBalanceEnabled,
          contextSettings?.nfcBalanceEnabledOverride
        )
      },
      defaultProvider: base.defaultProvider,
      pixExpirationMinutes: clampPixExpiration(base.pixExpirationMinutes),
      maxInstallments: clampInstallments(
        contextSettings?.maxInstallmentsOverride ??
  ...
  async assertContextCanEnableMethods({
    organizationId,
    pixEnabledOverride,
    creditEnabledOverride,
    debitEnabledOverride,
    cashEnabledOverride,
    nfcBalanceEnabledOverride
  }: {
    organizationId: string
    ...
    const invalid = blocked.find(([, override, enabled]) =>
      override === true && !enabled
    )
