...
export function encryptPaymentCredentials(value) {
...
export function decryptPaymentCredentials(encryptedPayload) {
    const [version, ivBase64, authTagBase64, encryptedBase64] = encryptedPayload.split('.');
    if (version !== KEY_VERSION || !ivBase64 || !authTagBase64 || !encryptedBase64) {
        throw new Error('Invalid encrypted credentials payload');
    }
    const decipher = crypto.createDecipheriv(ALGORITHM, getSecret(), Buffer.from(ivBase64, 'base64url'));
