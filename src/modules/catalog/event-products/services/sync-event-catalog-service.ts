...
type SkippedItem = SyncItem & {
  reason: SkippedReason
}

interface SyncEventCatalogServiceRequest {
...
function isInternalComboCategory(categoryName: string) {
...
export class SyncEventCatalogService {
  async execute({
    ...
    eventId,
    dryRun
  }: SyncEventCatalogServiceRequest) {
    if (
    ...
    const result = await prisma.$transaction(async tx => {
      ...
      const existingCatalogProductIds = new Set(
        existingEventProducts.map(eventProduct => eventProduct.catalogProductId)
      )

      const skippedItems: SkippedItem[] = []
      const eligibleItems: SyncItem[] = []
      ...
      const productsToCreate = eligibleItems.filter(
        item => !existingCatalogProductIds.has(item.productId)
      )
