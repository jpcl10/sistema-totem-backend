...
interface BulkCreateEventProductInput {
...
interface BulkCreateEventProductsServiceRequest {
  organizationId: string
  userId: string
  eventId: string
  products: BulkCreateEventProductInput[]
}
...
export class BulkCreateEventProductsService {
  async execute({
    ...
    eventId,
    products
  }: BulkCreateEventProductsServiceRequest) {
    ...
    const uniqueCatalogProductIds = [
      ...new Set(
        normalizedProducts.map(product => product.catalogProductId)
      )
    ]
    ...
    const result = await prisma.$transaction(async tx => {
      ...
      const existingCatalogProductIds = new Set(
        existing.map(eventProduct => eventProduct.catalogProductId)
      )

      const productsToCreate = normalizedProducts.filter(
        product => !existingCatalogProductIds.has(product.catalogProductId)
      )
      ...
      await createAuditLogService.execute({
        ...
        metadata: {
          created: result.created.map(
            eventProduct => eventProduct.catalogProductId
          ),
          existing: result.existing.map(
            eventProduct => eventProduct.catalogProductId
          )
        }
      })
