import { z } from 'zod';
const processingModes = ['BULLMQ', 'LEGACY_POLLING', 'DISABLED'];
const boolSchema = z
    .string()
    .optional()
...
const configuredMode = process.env.PRINT_PROCESSING_MODE ??
...
export const redisConfig = {
    ...
    connectTimeoutMs: parseIntegerEnv(process.env.REDIS_CONNECT_TIMEOUT_MS, 10000),
    maxRetriesPerRequest: parseIntegerEnv(process.env.REDIS_MAX_RETRIES_PER_REQUEST, 2)
};
export const printProcessingConfig = {
    configuredMode,
    redisFailureGraceMs: parseIntegerEnv(process.env.PRINT_REDIS_FAILURE_GRACE_MS, 30000),
    concurrency: Math.max(1, parseIntegerEnv(process.env.PRINT_QUEUE_CONCURRENCY, 2)),
    jobTimeoutMs: Math.max(1000, parseIntegerEnv(process.env.PRINT_JOB_TIMEOUT_MS, 30000)),
    staleLockMs: Math.max(5000, parseIntegerEnv(process.env.PRINT_JOB_STALE_LOCK_MS, 120000)),
    pollingIntervalMs: Math.max(1000, parseIntegerEnv(process.env.PRINT_LEGACY_POLLING_INTERVAL_MS, 3000))
};
