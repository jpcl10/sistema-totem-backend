...
function buildRedisOptions(role) {
    return {
        ...
        maxRetriesPerRequest: role === 'bullmq-worker' ? null : redisConfig.maxRetriesPerRequest,
        enableReadyCheck: true
    };
}
function registerListeners(client, role) {
    client.on('connect', () => {
...
export function createRedisConnection(role) {
...
export async function getRedisClient(role = 'general') {
    if (!redisConfig.enabled || !redisConfig.url) {
        return null;
    }
...
export async function closeRedisConnections() {
    await Promise.allSettled(Array.from(clients.values()).map(async (client) => {
        await client.quit();
    }));
    clients.clear();
}
