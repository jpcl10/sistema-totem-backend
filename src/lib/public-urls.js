...
export function getSocketPublicUrl() {
    return normalizeBaseUrl(process.env.SOCKET_PUBLIC_URL ??
...
export function buildPublicEventPath({ organizationSlug, eventSlug }) {
    return `/e/${organizationSlug}/${eventSlug}`;
}
export function buildPublicEventUrl({ organizationSlug, eventSlug }) {
    return buildPublicUrl(getFrontendUrl(), buildPublicEventPath({
        organizationSlug,
        eventSlug
    }));
}
