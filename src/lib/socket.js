...
import { corsAllowedHeaders, corsAllowedMethods, validateSocketCorsOrigin } from './cors.js';
import { redisConfig } from '../shared/config/redis.js';
import { createRedisConnection } from '../infra/redis/redis-client.js';
import { AmbiguousEventSlugError, resolveCanonicalPublicEvent, resolveLegacyPublicEventSlug } from '../modules/events/services/public-event-resolver.js';
export let io;
export function setSocketServerForTests(socketServer) {
    io = socketServer;
}
...
function readSocketOrganizationId(socket) {
    return (readSingleValue(socket.handshake.auth?.organizationId) ??
        readSingleValue(socket.handshake.headers?.['x-organization-id']));
}
function readPublicEventRoomPayload(payload) {
    ...
    if (payload && typeof payload === 'object') {
        const value = payload;
        const eventSlug = typeof value.eventSlug === 'string'
            ? value.eventSlug
...
async function resolveSocketTenantContext(socket) {
    ...
    io.on('connection', async (socket) => {
        ...
        try {
            ...
            if (tenantContext) {
                ...
                logger.warn({ socketId: socket.id }, 'Socket connected without tenant context; operational rooms disabled');
            }
        }
        catch (error) {
            logger.warn({ socketId: socket.id, error }, 'Socket tenant context resolution failed; operational rooms disabled');
        }
        socket.on('join-event-room', async (eventId) => {
            const tenantContext = socket.data.tenantContext;
            if (!tenantContext?.organizationId) {
                logger.warn({ socketId: socket.id, eventId }, 'Socket event room join denied without tenant context');
                return;
            }
        ...
        socket.on('join-call-screen-store', async (slug) => {
            ...
            if (!store) {
                logger.warn({ socketId: socket.id, slug }, 'Public call screen store room join denied because store was not found');
                return;
            }
        ...
        socket.on('join-call-screen-event', async (payload) => {
            ...
            try {
                ...
                if (!event) {
                    logger.warn({ socketId: socket.id, slug: publicEventPayload.slug }, 'Public call screen event room join denied because event was not found');
                    return;
                }
